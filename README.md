...
프로젝트에는 신경 신호 데이터를 모방한 다양한 샘플 데이터셋이 포함되어 있습니다:

1. **정상 신경 신호**: 정상적인 신경 활동을 모사한 데이터
2. **손상된 신경 신호**: 손상된 신경의 감소된 활동 패턴을 보여주는 데이터
3. **자극 반응 신호**: 전기자극 후 신경의 반응을 모사한 데이터
4. **잡음이 많은 신호**: 다양한 노이즈 유형을 포함한 신호 데이터
5. **다채널 신경 신호**: 여러 전극에서 동시에 기록된 신호를 모사한 데이터

샘플 데이터는 다음 경로에서, CSV, NPY, HDF5 형식으로 제공됩니다:
- `data/samples/`
...
자세한 데이터 설명은 `data/README.md` 파일을 참조하세요.

## GitHub Actions CI/CD 파이프라인

이 프로젝트는 GitHub Actions를 통해 다음과 같은 CI/CD 파이프라인을 제공합니다:

1. **Python 테스트**: 파이썬 코드의 단위 테스트 및 통합 테스트 실행
   - 다양한 파이썬 버전(3.8, 3.9, 3.10)에서 테스트
   - 테스트 커버리지 보고서 생성

2. **데이터 검증**: 샘플 데이터 파일의 유효성 검사
   - CSV 파일 형식 및 내용 검증
   - 신호 값 범위 및 형식 확인
   - 샘플 데이터의 시각화 및 플롯 생성

워크플로우 파일은 `.github/workflows/` 디렉토리에 있습니다:
- `python-tests.yml`: 파이썬 코드 테스트 워크플로우
- `data-validation.yml`: 데이터 검증 워크플로우
...
5. **실시간 처리 개선**: 실시간 신호 처리 및 자극 조정 기능 강화
6. **사용자 인터페이스 확장**: 연구자 및 임상의를 위한 직관적인 UI 개발
